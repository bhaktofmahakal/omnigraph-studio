...
import { FileCode, Split, Code, GitCommit, CheckCircle2, ShieldAlert, Sparkles } from 'lucide-react';

// Dynamically import Monaco Editor to prevent SSR window issues
const MonacoEditor = dynamic(() => import('@monaco-editor/react'), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-full bg-[#090a0f] text-zinc-500 font-mono text-xs">
      Loading Monaco Editor Subsystem...
    </div>
  ),
});
...
export const CodeEditor: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#090a0f] border border-[#222638] rounded-xl overflow-hidden shadow-2xl">
      {/* File Tabs & Mode Controls */}
      <div className="flex items-center justify-between px-2 pt-2 bg-[#0e1017] border-b border-[#222638]">
        {/* Tab List */}
          ...
          {files.map((file) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-lg text-xs font-mono transition-all border-t border-x ${
                  isActive
              ...
              >
                ...
                {hasPending && (
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" title="Pending Surgical Diff" />
                )}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-1.5 pb-1">
          ...
          {pendingHunksCount > 0 && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-mono font-medium transition-all shadow-[0_0_10px_rgba(245,158,11,0.2)]"
            >
              <ShieldAlert className="w-3.5 h-3.5 text-amber-400" />
              <span>Review ({pendingHunksCount} Hunks)</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-3 py-1 bg-[#0e1017] border-t border-[#222638] font-mono text-[10px] text-zinc-500">
        <div className="flex items-center gap-3">
          <span>PATH: {activeFile?.path}</span>
          <span>LANG: {activeFile?.language.toUpperCase()}</span>
          <span>LINES: {activeFile?.currentCode.split('\n').length}</span>
        </div>
